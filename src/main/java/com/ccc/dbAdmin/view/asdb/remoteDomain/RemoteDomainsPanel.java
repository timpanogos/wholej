 ...
 */
package org.emitdo.research.app.dbAdmin.view.asdb.remoteDomain;
...
public class RemoteDomainsPanel extends JPanel implements ActionListener, ListSelectionListener, MouseListener, AppliedCallback
{
...
//    private AuthNodeTableModel tableModel;
    ...
    public RemoteDomainsPanel(AsManagerFrame asManager)
    {
...
//        mainPanel.setBorder(new TitledBorder("Authentication Nodes"));
        ...
        asManager.getRemoteDomainNodes(oidListModel, permListModel);
    }
    ...
    {
        if(ok)
            asManager.refreshDialog(AsManagerFrame.RemoteDomainsDialog);
    }
...
//                new AuthenticationDialog(asManager, event.getComponent(), this, index, false);
    ...
    public void valueChanged(ListSelectionEvent event)
    {
        ...
        if(src == oidList)
        {
            ...
            {
                if(oid.equals(((RemoteDomainInfo)permListModel.get(i)).oid))
        ...
        if(src == permList)
        {
            ...
            if(set.length == 1)
            {
                ...
                for(int i=0; i < size; i++)
                {
                    if(((RemoteDomainInfo)set[0]).oid.equals(oidListModel.get(i)))
                    {
    ...
    public static class RemoteDomainsDialog extends DialogBase
    {
        private final AsManagerFrame asManager;
        public RemoteDomainsDialog(AsManagerFrame asManager, Component source)
        {
            super(
                    asManager, 
                    asManager.getFramePreference(AsManagerFrame.RemoteDomainsDialog),
                    "Remote domain nodes: " + asManager.getDomain(), false);
            this.asManager = asManager;
            setContentPane(new RemoteDomainsPanel(asManager));
            pack();
            setVisible(true);
        ...
        public void refresh()
        {
            ((RemoteDomainsPanel)getContentPane()).refresh();
        }
        ...
        public void windowClosed(WindowEvent e)
        {
            ...
            asManager.dialogClosed(AsManagerFrame.RemoteDomainsDialog, this);
        }
    }
}
