...
public class BackupConnectionPanel extends JPanel implements ActionListener, DocumentListener, SwappedFocusListener, WindowFocusListener
{
    ...
    private JRadioButton oracleBackupRadio; 
    private JRadioButton jdbBackupRadio; 
    private JRadioButton pgBackupRadio; 
    
    private JCheckBox oracleBackupCheck;
    private JCheckBox jdbBackupCheck;
    private JCheckBox pgBackupCheck;
    ...
    public BackupConnectionPanel(DbAdmin dbAdmin)
    {
        ...
        mainPanel.setBorder(new TitledBorder("Backup Connections"));
        ...
        JLabel oracleResLabel = FrameBase.getLabel("Backup:", maxWidth);
        JLabel jdbResLabel = FrameBase.getLabel("Backup:", maxWidth);
        JLabel pgResLabel = FrameBase.getLabel("Backup:", maxWidth);
        ...
        oracleBackupRadio = FrameBase.addRadioButtonToPanel(oracleResLabel, true, true, width, -1, "Oracle Backup DB connection", rowPanel, this);
        oracleBackupCheck = FrameBase.addCheckBoxToPanel(oracleResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.Oracle, oracleBackupRadio, oracleBackupCheck);
        panel.add(rowPanel);
        
        ButtonGroup group = new ButtonGroup();
        group.add(oracleBackupRadio);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        jdbBackupRadio = FrameBase.addRadioButtonToPanel(jdbResLabel, false, true, width, -1, "JavaDB Backup DB connection", rowPanel, this);
        jdbBackupCheck = FrameBase.addCheckBoxToPanel(jdbResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.JavaDb, jdbBackupRadio, jdbBackupCheck);
        panel.add(rowPanel);
        
        group.add(jdbBackupRadio);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        pgBackupRadio = FrameBase.addRadioButtonToPanel(pgResLabel, false, true, width, -1, "Postgres Backup DB connection", rowPanel, this);
        pgBackupCheck = FrameBase.addCheckBoxToPanel(pgResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.Postgres, pgBackupRadio, pgBackupCheck);
        panel.add(rowPanel);
        group.add(pgBackupRadio);
    ...
    {
        ConnectionData ci = control.getConnection(StorageType.Backup, vtype, null, ConnType.Jdbc);
        ci.checkBox = check;
        ci.radioButton = radio;
        ci = control.getConnection(StorageType.Backup, vtype, null, ConnType.Web);
        ci.checkBox = check;
        ci.radioButton = radio;
    }
    ...
    private void setState()
    {
        for(int j=0; j < 3; j++)
        {
            ...
            if(!jdbcRadio.isSelected())
                ctype = ConnType.Web;
            ConnectionData ci = control.getConnection(StorageType.Backup, vtype, null, ctype);
    ...
    private void adjustForSelection()
    {
        ...
        boolean jdb = false;
        if(oracleBackupRadio.isSelected()) 
        {
            vtype = VendorType.Oracle;
        }else
        if(jdbBackupRadio.isSelected())
        {
            vtype = VendorType.JavaDb;
            jdb = true;
        }else
        if(pgBackupRadio.isSelected())
        {
            vtype = VendorType.Postgres;
            jdb = false;
        }
        connection = control.getConnection(StorageType.Backup, vtype, null, ctype);
        painted.set(false);
