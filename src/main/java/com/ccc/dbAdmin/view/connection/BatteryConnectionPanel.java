...
public class BatteryConnectionPanel extends JPanel implements ActionListener, DocumentListener, SwappedFocusListener, WindowFocusListener
{
    ...
    public BatteryConnectionPanel(DbAdmin dbAdmin)
    {
        ...
        mainPanel.setBorder(new TitledBorder("Battery Connections"));
        ...
        JLabel jdbBatteryLabel = FrameBase.getLabel("Battery:", maxWidth);
        ...
        JLabel jdbBatteryConnectedLabel = FrameBase.getLabel("Connected:", maxWidth);
        ...
        associateState(VendorType.Oracle, oracleBatteryRadio, oracleBatteryCheck);
        panel.add(rowPanel);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        jdbBatteryRadio = FrameBase.addRadioButtonToPanel(jdbBatteryLabel, false, true, width, -1, "JavaDB Battery Power DB connection", rowPanel, this);
        jdbBatteryCheck = FrameBase.addCheckBoxToPanel(jdbBatteryConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.JavaDb, jdbBatteryRadio, jdbBatteryCheck);
        panel.add(rowPanel);
        
        group.add(jdbBatteryRadio);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        pgBatteryRadio = FrameBase.addRadioButtonToPanel(pgBatteryLabel, true, true, width, -1, "Postgres Battery Power DB connection", rowPanel, this);
        pgBatteryCheck = FrameBase.addCheckBoxToPanel(pgBatteryConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.Postgres, pgBatteryRadio, pgBatteryCheck);
        panel.add(rowPanel);
    ...
    {
        ConnectionData ci = control.getConnection(StorageType.Battery, vtype, null, ConnType.Jdbc);
        ci.checkBox = check;
        ci.radioButton = radio;
        ci = control.getConnection(StorageType.Battery, vtype, null, ConnType.Web);
        ci.checkBox = check;
        ci.radioButton = radio;
    }
    ...
    private void setState()
    {
        for(int j=0; j < 2; j++)
        {
            ...
            if(!jdbcRadio.isSelected())
                ctype = ConnType.Web;
            ConnectionData ci = control.getConnection(StorageType.Battery, vtype, null, ctype);
    ...
    private void adjustForSelection()
    {
        ...
        connection = control.getConnection(StorageType.Battery, vtype, null, ctype);
        painted.set(false);
