...
public class ResConnectionPanel extends JPanel implements ActionListener, DocumentListener, SwappedFocusListener, WindowFocusListener
{
    ...
    public ResConnectionPanel(DbAdmin dbAdmin)
    {
        ...
        mainPanel.setBorder(new TitledBorder("Residential Platform Connections"));
        ...
        oracleResRadio = FrameBase.addRadioButtonToPanel(oracleResLabel, true, true, width, -1, "Oracle residential platform DB connection", rowPanel, this);
        oracleResCheck = FrameBase.addCheckBoxToPanel(oracleResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.Oracle, oracleResRadio, oracleResCheck);
        panel.add(rowPanel);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        jdbResRadio = FrameBase.addRadioButtonToPanel(jdbResLabel, false, true, width, -1, "JavaDB residential platform DB connection", rowPanel, this);
        jdbResCheck = FrameBase.addCheckBoxToPanel(jdbResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.JavaDb, jdbResRadio, jdbResCheck);
        panel.add(rowPanel);
        ...
        rowPanel = new JPanel();
        rowPanel.setLayout(new BoxLayout(rowPanel, BoxLayout.X_AXIS));
        pgResRadio = FrameBase.addRadioButtonToPanel(pgResLabel, false, true, width, -1, "Postgres residential platform DB connection", rowPanel, this);
        pgResCheck = FrameBase.addCheckBoxToPanel(pgResConnectedLabel, false, false, width, -1, "Currently connected", rowPanel);
        associateState(VendorType.Postgres, pgResRadio, pgResCheck);
        panel.add(rowPanel);
    ...
    {
        ConnectionData ci = control.getConnection(StorageType.Res, vtype, null, ConnType.Jdbc);
        ci.checkBox = check;
        ci.radioButton = radio;
        ci = control.getConnection(StorageType.Res, vtype, null, ConnType.Web);
        ci.checkBox = check;
        ci.radioButton = radio;
    }
    ...
    private void setState()
    {
        for(int j=0; j < 2; j++)
        {
            ...
            if(!jdbcRadio.isSelected())
                ctype = ConnType.Web;
            ConnectionData ci = control.getConnection(StorageType.Res, vtype, null, ctype);
    ...
    private void adjustForSelection()
    {
        ...
        connection = control.getConnection(StorageType.Res, vtype, null, ctype);
        painted.set(false);
